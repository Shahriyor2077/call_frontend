...
const COLUMNS = [
  { key: 'NEW',         label: 'Yangi',      dot: 'bg-blue-500',   statBg: 'bg-blue-500' },
  { key: 'IN_PROGRESS', label: 'Muloqotda',  dot: 'bg-amber-500',  statBg: 'bg-amber-500' },
  { key: 'ENROLLED',    label: 'Yozildi',    dot: 'bg-green-500',  statBg: 'bg-green-500' },
  { key: 'NOT_COME',    label: 'Kelmadi',    dot: 'bg-gray-400',   statBg: 'bg-gray-400' },
  { key: 'REJECTED',    label: 'Rad etdi',   dot: 'bg-red-500',    statBg: 'bg-red-500' },
];
...
export default function OperatorLeadsPage() {
  ...
  const activeFilterCount = [filterSearch, filterSource].filter(Boolean).length;
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-5">
        {COLUMNS.map(col => {
          ...
          return (
            <button
              key={col.key}
              onClick={() => router.push(`/operator/leads/status/${col.key}`)}
              className="p-4 rounded-2xl border border-gray-100 bg-white flex items-center justify-between transition-all shadow-sm hover:shadow-md cursor-pointer"
            >
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3 mb-4 flex items-center gap-3 flex-wrap">
        ...
        <div className="relative" ref={filterRef}>
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 border rounded-xl text-sm transition-colors cursor-pointer ${filterOpen || filterSource ? 'border-indigo-400 bg-indigo-50 text-indigo-700' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}
          >
          ...
          {filterOpen && (
            <div className="absolute top-full left-0 mt-2 bg-white border border-gray-200 rounded-2xl shadow-2xl z-40 w-64 p-4 space-y-3">
              ...
              <div className="flex gap-2 pt-1">
                <button
                  onClick={() => { setFilterSource(''); setFilterOpen(false); }}
                  className="flex-1 py-2 border border-gray-200 rounded-xl text-sm text-gray-600 hover:bg-gray-50 transition-colors cursor-pointer"
                >
                  Tozalash
                </button>
                <button
        ...
        {activeFilterCount > 0 && (
          <button
            onClick={() => { setFilterSearch(''); setFilterSource(''); }}
