...
export default function AdminStudentsPage() {
  ...
  const [modal, setModal] = useState<'create' | 'enroll' | 'edit' | 'changeGroup' | 'delete' | null>(null);
  ...
  function toggleAll() {
    ...
    else setSelectedIds(new Set(byFilter.map(s => s.id)));
  }
  ...
  return (
    <div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            {paginated.map((s, i) => {
              ...
              return (
                <tr key={s.id} className={`hover:bg-gray-50/60 transition-colors ${selectedIds.has(s.id) ? 'bg-indigo-50/40' : ''}`}>
      ...
      {/* Bulk action bar */}
      {selectedIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-white border border-gray-200 rounded-2xl shadow-2xl px-4 py-3">
          <div className="flex items-center gap-2 bg-gray-100 rounded-xl px-3 py-1.5">
            <span className="text-sm font-semibold text-gray-700">{selectedIds.size} Tanlangan</span>
          ...
          <button
            onClick={() => {
              ...
              const firstStudent = byFilter.find(s => ids.includes(s.id));
              if (firstStudent) {
          ...
          >
            <UserPlus size={14} /> Guruhga biriktirish
          </button>
          <button
      ...
      <Modal open={modal === 'delete'} onClose={() => setModal(null)} title="Talabani o'chirish" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            <span className="font-semibold text-gray-900">{selectedStudent?.name}</span> talabani o&apos;chirishni xohlaysizmi?
          </p>
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
            <p className="text-sm text-amber-800">⚠️ Bu amal qaytarilmaydi. Talaba va barcha ma&apos;lumotlar o&apos;chiriladi.</p>
          </div>
          <div className="flex gap-3 pt-2">
            <Button variant="danger" onClick={() => void deleteStudent()} loading={loading} className="flex-1">O&apos;chirish</Button>
            <Button variant="secondary" onClick={() => setModal(null)} className="flex-1">Bekor</Button>
          </div>
        </div>
      </Modal>
      ...
      <Modal open={bulkDeleteConfirm} onClose={() => setBulkDeleteConfirm(false)} title="O'chirishni tasdiqlash" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            <span className="font-semibold text-gray-900">{selectedIds.size} ta</span> talabani o&apos;chirishni xohlaysizmi?
          </p>
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
            <p className="text-sm text-amber-800">⚠️ Bu amal qaytarilmaydi. Barcha ma&apos;lumotlar o&apos;chiriladi.</p>
          </div>
          <div className="flex gap-3 pt-2">
            <Button variant="danger" onClick={() => void bulkDelete()} loading={bulkLoading} className="flex-1">Ha, o&apos;chirish</Button>
            <Button variant="secondary" onClick={() => setBulkDeleteConfirm(false)} className="flex-1">Bekor</Button>
          </div>
        </div>
      </Modal>
